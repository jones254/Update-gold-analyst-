    ...
    try:
        ...
        if rows:
            ...
            def _color(val):
                ...
                return ""
            st.dataframe(
                fdf.style.applymap(_color, subset=["Label"]),
...
with tab_signals:
    ...
    if not multi_tf_ready:
        ...
        # Live entry setup — chart with entry / SL / TP levels
        st.markdown("##### 🎯 Live entry setup — medium horizon")
        st.caption(
            "Gold 15m chart with the medium-horizon (1h) signal's entry / "
            "stop / target.  The shaded band is the risk zone; the green band is the reward."
        )
        try:
            # Use 1h data (the "medium horizon") as the basis for the chart,
            # then overlay the entry/SL/TP from res.
            base_df = multi_tf_data.get("1h", {}).get("gold")
            if base_df is None or len(base_df) < 5:
                base_df = data.get("gold")
            if base_df is not None and len(base_df) > 0:
                tail = base_df.tail(120).copy()
                fig_e = go.Figure()
                # Candlesticks
                ...
                if res.entry is not None:
                    ...
                    fig_e.add_hline(
                        y=res.entry, line_dash="solid", line_color=line_color, line_width=2,
                        annotation_text=f"Entry {res.entry:,.2f}",
                        annotation_position="right",
                        annotation_font_color=line_color,
                    )
                    # Risk band (entry → stop)
                    fig_e.add_hrect(
                        y0=min(res.entry, res.stop), y1=max(res.entry, res.stop),
                        fillcolor="rgba(178,34,34,0.12)", line_width=0,
                    )
                    fig_e.add_hline(
                        y=res.stop, line_dash="dash", line_color="#B22222", line_width=1.5,
                        annotation_text=f"Stop {res.stop:,.2f}",
                        annotation_position="right",
                        annotation_font_color="#B22222",
                    )
                    # Reward band (entry → target)
                    fig_e.add_hrect(
                        y0=min(res.entry, res.target), y1=max(res.entry, res.target),
                        fillcolor="rgba(46,139,87,0.12)", line_width=0,
                    )
                    fig_e.add_hline(
                        y=res.target, line_dash="dash", line_color="#2E8B57", line_width=1.5,
                        annotation_text=f"Target {res.target:,.2f}",
                        annotation_position="right",
                ...
                st.info("Gold chart data not available.")
        except Exception as e:
