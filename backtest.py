...
def run_backtest(
    ...
    `data` is the full multi-market dict from the DataSource — we look up
    `data["gold"]["Close"]` as the target instrument.
    """
    ...
    # ----- 3. Forward return of GOLD ----------------------------------------
    gold_close = data["gold"]["Close"].reindex(df.index)
    fwd = gold_close.shift(-bt.holding_period) / gold_close - 1.0
    df["fwd_return"] = fwd
    ...
    daily_ret = gold_close.pct_change().fillna(0.0)
