...
def _fetch_multi_tf(source_name: str, api_key: str):
    ...
    return fetch_multi_timeframe(cfg, intervals=("15m", "1h", "1d"))
...
long_tf_ready = (
    "gold" in multi_tf_data.get("1d", {}) and len(multi_tf_data["1d"].get("gold", pd.DataFrame())) > 30
)
...
with tab_signals:
    ...
    if not multi_tf_ready:
        ...
        # ---- Per-horizon entry setup -----------------------------------
        # Build the per-horizon (short/medium/long) entry setups.  Each
        # horizon has its own data, its own EMA periods, and its own
        # entry/SL/TP computed from that horizon's chart.
        ...
        horizon_meta = {
            "short":  {"tf": "15m", "data": multi_tf_data.get("15m", {}).get("gold"),
                       "label": "SHORT · EMA 10/20 · 15m",  "use": "Use short for entry"},
            "medium": {"tf": "1h",  "data": multi_tf_data.get("1h",  {}).get("gold"),
                       "label": "MEDIUM · EMA 20/50 · 1h",  "use": "Use medium for entry"},
            "long":   {"tf": "1d",  "data": multi_tf_data.get("1d",  {}).get("gold"),
                       "label": "LONG · EMA 50/200 · 1d",   "use": "Use long for entry"},
        }
        # Filter out horizons without data
        available_horizons = [h for h, m in horizon_meta.items() if m["data"] is not None and len(m["data"]) > 30]
        # Fall back to the legacy data["gold"] for any missing tf
        for h, m in horizon_meta.items():
            if (m["data"] is None or len(m["data"]) <= 30) and h == "short" and "gold" in data:
                horizon_meta[h]["data"] = data["gold"]

        side = 1 if res.signal == "BUY" else (-1 if res.signal == "SELL" else 0)

        # Render 3 cards: short / medium / long
        hcols = st.columns(3)
        for i, h in enumerate(("short", "medium", "long")):
            with hcols[i]:
                ...
                if gold_df is None or len(gold_df) < 30:
                    ...
                    continue
                # Compute per-horizon entry/SL/TP
                setup = horizon_entry_sltp(gold_df, side, sig_cfg, h)
                # Forecast score/label for this horizon
                fc_label = "—"
                fc_score = 0.0
                fc_conf  = 0.0
                try:
                    ...
                    pass
                # Color the card by forecast label
                ...
                # Use-this-horizon button (only on non-active)
                if not is_active:
                    if st.button(meta["use"], key=f"use_{h}", use_container_width=True):
                        st.session_state.active_horizon = h
                        st.rerun()
                else:
                    st.button("✓ Selected", key=f"sel_{h}", use_container_width=True, disabled=True)

        # Reference table below the cards
        st.markdown("")
        try:
            ref_rows = []
            for h in ("short", "medium", "long"):
                ...
                if gold_df is None or len(gold_df) < 30:
                    ...
                    continue
                setup = horizon_entry_sltp(gold_df, side if side else 1, sig_cfg, h)
                fc_label = "—"
                try:
                    fc_label = str(f_labels[h].iloc[-1])
                except Exception:
            ...
            def _color(val):
                ...
                return ""
            st.dataframe(rdf.style.map(_color, subset=["Forecast"]), use_container_width=True, height=110)
        except Exception as e:
        ...
        if active_df is None or len(active_df) < 5:
            ...
            try:
                setup = horizon_entry_sltp(active_df, side if side else 1, sig_cfg, active)
                ...
                # Caption describing forecast + bias
                fc_label = "—"
                fc_score = 0.0
                fc_conf  = 0.0
                try:
                ...
                ent_v  = f"{setup['entry']:.1f}" if setup["entry"] else "—"
                st.caption(
                ...
                # Candlestick chart with EMA + entry/SL/TP bands
                ...
                if setup["ema_fast"] is not None:
                    # Recompute EMA series for the visible window
